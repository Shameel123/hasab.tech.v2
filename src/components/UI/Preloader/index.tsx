...
const Preloader = ({
  ...
  useEffect(() => {
    ...
    tl.to(spans.current, {
      ...
      stagger: 0.04, // Stagger duration (0.2 seconds delay between each span)
    });
    ...
    tl.to(secondOverlayRef.current, {
      ...
      duration: 1,
      delay: -0.5, // Adjust this delay as needed to fine-tune the timing
    });
  }, [setComplete]);
  ...
  return (
    <>
      <Wrapper ref={wrapperRef}>
        <Inner>
          <Image ref={imageRef} src={'/images/hash-icon.png'} width={300} height={300} alt="import icon" />
          <div>
            {word.map((t, i) => (
              <div
                key={i}
                // ref={(element) => (spans.current[i] = element)} 
                ref={(element: HTMLDivElement | null) => {// Assign ref to each span
